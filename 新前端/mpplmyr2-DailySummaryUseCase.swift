...
final class DailySummaryUseCase {
    ...
    func loadDashboard(for date: Date = Date(), modelContext: ModelContext? = nil) async throws -> DashboardSummary {
        ...
        if let modelContext {
            ...
            if let saved = snapshots.first(where: { calendar.isDate($0.date, inSameDayAs: yesterday) })?.strainScore {
                yesterdayStrain = saved
            } else {
                // Fallback: if no snapshot exists for yesterday, use a neutral estimate (50)
                // to avoid inflating recovery by omitting the prior_strain factor entirely.
                yesterdayStrain = 50
            }
        } else {
            yesterdayStrain = nil
        ...
        // Unify baseline with SD data source: compute baseline HRV/RHR from the same
        // daily averages used for z-score SD, so scoring is internally consistent.
        var unifiedContext = context
        if !hrvHistory.isEmpty {
            let hrvMean = hrvHistory.reduce(0, +) / Double(hrvHistory.count)
            unifiedContext.recoveryBaseline = RecoveryMetricSummary(
                hrvMilliseconds: hrvMean,
                restingHeartRate: unifiedContext.recoveryBaseline.restingHeartRate,
                sleepHeartRate: unifiedContext.recoveryBaseline.sleepHeartRate,
                respiratoryRate: unifiedContext.recoveryBaseline.respiratoryRate
            )
        }
        if !rhrHistory.isEmpty {
            let rhrMean = rhrHistory.reduce(0, +) / Double(rhrHistory.count)
            unifiedContext.recoveryBaseline = RecoveryMetricSummary(
                hrvMilliseconds: unifiedContext.recoveryBaseline.hrvMilliseconds,
                restingHeartRate: rhrMean,
                sleepHeartRate: unifiedContext.recoveryBaseline.sleepHeartRate,
                respiratoryRate: unifiedContext.recoveryBaseline.respiratoryRate
            )
        }
    ...
    private func backfillSleepHistoryIfNeeded(modelContext: ModelContext, now: Date) async {
        ...
        for episode in episodes {
            ...
            let dashboard = DashboardSummary.healthKit(
                context: context,
                strainScoreYesterday: nil, // Backfilled: prior strain unavailable
