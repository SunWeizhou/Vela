...
final class DashboardViewModel: ObservableObject {
    ...
    func goToToday() {
        ...
        selectedDate = Date()
        lastRefreshDate = nil
        lastRefreshedSelectedDate = nil
    }

    func resetDateIfNeeded() {
    ...
    private var lastRefreshedSelectedDate: Date?
    ...
    func refresh(modelContext: ModelContext? = nil, force: Bool = false) async {
        // Dedup: skip if already refreshed for the same date within the debounce window
        if !force,
           let lastRefresh = lastRefreshDate,
           let lastDate = lastRefreshedSelectedDate,
           Calendar.current.isDate(lastDate, inSameDayAs: selectedDate),
           Date().timeIntervalSince(lastRefresh) < refreshDebounceInterval {
            return
        }

        // If a refresh is already in flight for the same date, await it
        ...
        refreshTask = Task { [weak self] in
            ...
            isLoading = true
            errorMessage = nil
            let dateForRefresh = selectedDate
            do {
                dashboard = try await useCase.loadDashboard(for: dateForRefresh, modelContext: modelContext)
                lastUpdated = Date()
                lastRefreshDate = Date()
                lastRefreshedSelectedDate = dateForRefresh
