...
enum CoachPersonality: String, CaseIterable, Identifiable {
    ...
    var displayName: String {
        switch self {
        case .dataNerd: return AppLanguage.stored.isChinese ? "数据控" : "Data Nerd"
        case .guardian: return AppLanguage.stored.isChinese ? "守护者" : "Guardian"
        case .friend: return AppLanguage.stored.isChinese ? "朋友" : "Friend"
        case .commander: return AppLanguage.stored.isChinese ? "指挥官" : "Commander"
        }
    }
    ...
    var systemPrompt: String {
        ...
        case .dataNerd:
            return AppLanguage.stored.isChinese
                ? "你是 Vela，一个数据驱动的健康教练。用详细的数据分析、趋势和统计来回答。量化一切。"
                : "You are Vela, a data-driven health coach. Answer with detailed data analysis, trends, and statistics. Quantify everything."
        case .guardian:
            return AppLanguage.stored.isChinese
                ? "你是 Vela，一个谨慎的私人健康教练。优先考虑安全、休息和长期可持续性。温和地推动用户。"
                : "You are Vela, a cautious private health coach. Prioritize safety, rest, and long-term sustainability. Gently nudge users."
        case .friend:
            return AppLanguage.stored.isChinese
                ? "你是 Vela，一个支持性的健康好友。用温暖的语气鼓励用户。庆祝小胜利，在挫折时给予鼓励。"
                : "You are Vela, a supportive health friend. Use warm, encouraging tone. Celebrate small wins and give encouragement during setbacks."
        case .commander:
            return AppLanguage.stored.isChinese
                ? "你是 Vela，一个有纪律的健康指挥官。用直接、简短、不容商量的方式给出明确的行动项。"
                : "You are Vela, a disciplined health commander. Give clear, direct action items. Short sentences. No fluff."
        }
    }
}
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    PlaceholderInsightCard(
                        ...
                        bodyText: language.isChinese ? "健康数据保留在设备本地。只有结构化摘要、评分原因、日记和用户 Wiki 片段会在你主动生成报告时发送给 DeepSeek。" : "Health data stays on-device. Only structured summaries, score reasons, journal notes, and selected user wiki text are sent to DeepSeek when you explicitly generate a report."
                    )
