...
final class CoachChatVM: ObservableObject {
    ...
    /// Captures a food photo, sends it to DeepSeek vision for nutritional analysis,
    ...
    func analyzeFoodPhoto(
        ...
        streamingContent = L10n.t("Analyzing your meal...", "正在分析你的餐食...")
        ...
        do {
            ...
            // Also auto-log the food entry
            let foodNames = result.foods.map { $0.name }
    ...
    private func buildChatMessages(
        ...
        let personality = CoachPersonality(rawValue: UserDefaults.standard.string(forKey: "vela_coach_personality") ?? "guardian") ?? .guardian
        ...
        3. **Personalized Training Plan Prescription Protocol**:
           ...
           - **Tool Usage**: You may call the generate_training_plan tool to get structured physiological state context to assist in generating the training plan.
...
private struct MiniBubble: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            MarkdownText(markdown: message.content + (message.isStreaming ? " ▊" : ""), font: .subheadline, color: VelaTheme.primaryText, isStreaming: message.isStreaming)
        }
        .padding(12)
