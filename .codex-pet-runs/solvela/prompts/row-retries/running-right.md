Create Codex pet row `running-right` for `solvela`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact whole-body solar-sail coding companion: a warm golden star-core face inside a small teal-and-white folded sail cloak, tiny rounded limbs, focused helpful expression, no text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
