...
struct VelaAppleInsightsView: View {
    ...
    private var dataCoverageContent: some View {
        VStack(alignment: .leading, spacing: VelaTheme.spaceLG) {
            ...
            VelaAppleSectionHeader(title: "Signal Quality")
    ...
    private var coverageHero: some View {
        HStack(alignment: .center, spacing: 20) {
            VelaAppleScoreRing(score: 85, color: VelaTheme.recovery, size: 100, lineWidth: 8, label: "Coverage")
        ...
        .appleHeroCard(accent: VelaTheme.recovery)
    }
    ...
    private func memoryProposal(title: String, evidence: String, source: String, target: String, confidence: String) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                VelaAppleStatusBadge(label: "Pattern", systemImage: "brain.head.profile", tint: VelaTheme.accent)
                Spacer()
            ...
            HStack(spacing: 10) {
                VelaApplePillButton(title: "Confirm", systemImage: "checkmark", role: .primary) {}
                VelaApplePillButton(title: "Reject", systemImage: "xmark", role: .secondary) {}
            }
        }
        .appleCard()
    }
        ...
        VStack(alignment: .leading, spacing: VelaTheme.spaceLG) {
            ...
            ForEach(trustLogItems, id: \.0) { (title, subtitle, status) in
                HStack(spacing: 12) {
                    ...
                    Spacer()
                    VelaAppleStatusBadge(label: status, tint: VelaTheme.recovery)
                }
                .padding(VelaTheme.spaceSM)
                .appleCard(radius: VelaTheme.radiusSM)
            }
